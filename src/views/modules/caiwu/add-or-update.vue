	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230329/eff4ea96c952463cb46b9c79d043ce56.gif) fixed,#f6f6f6"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="财务姓名" prop="caiwuxingming">
					<el-input v-model="ruleForm.caiwuxingming" placeholder="财务姓名" clearable  :readonly="ro.caiwuxingming"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="财务姓名" prop="caiwuxingming">
					<el-input v-model="ruleForm.caiwuxingming" placeholder="财务姓名" readonly></el-input>
				</el-form-item>
	...
	data() {
		...
		return {
			...
			rules: {
				...
				caiwuxingming: [
					{ required: true, message: '财务姓名不能为空', trigger: 'blur' },
				],
				xingbie: [
				],
				shouji: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `caiwu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.caiwuCrossAddOrUpdateFlag = false;
						 this.parent.search();
