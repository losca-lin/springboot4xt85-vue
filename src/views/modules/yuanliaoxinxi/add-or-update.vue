  ...
  <div class="addEdit-block"
    ...
    >
      <template>
        ...
        <el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.yuanliaobianhao"
                      label="原料编号" prop="yuanliaobianhao">
          <el-input v-model="ruleForm.yuanliaobianhao" placeholder="原料编号" readonly></el-input>
        </el-form-item>
        <el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="原料名称"
                      prop="yuanliaomingcheng">
          <el-input v-model="ruleForm.yuanliaomingcheng" placeholder="原料名称" clearable
                    :readonly="ro.yuanliaomingcheng"></el-input>
        </el-form-item>
        ...
        <el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.yuanliaotupian"
                      label="原料图片" prop="yuanliaotupian">
          <file-upload
              tip="点击上传原料图片"
              action="file/upload"
              :limit="3"
              :multiple="true"
              :fileUrls="ruleForm.yuanliaotupian?ruleForm.yuanliaotupian:''"
              @change="yuanliaotupianUploadChange"
          ></file-upload>
        </el-form-item>
        <el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.yuanliaotupian"
                      label="原料图片" prop="yuanliaotupian">
          <img v-if="ruleForm.yuanliaotupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;"
               v-bind:key="index" :src="ruleForm.yuanliaotupian.split(',')[0]" width="100" height="100">
          <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index"
               v-for="(item,index) in ruleForm.yuanliaotupian.split(',')" :src="$base.url+item" width="100"
               height="100">
        </el-form-item>
      ...
      <el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'" label="原料详情" prop="yuanliaoxiangqing">
        <editor
            style="min-width: 200px; max-width: 600px;"
            v-model="ruleForm.yuanliaoxiangqing"
            class="editor"
            action="file/upload">
        </editor>
      </el-form-item>
      <el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.yuanliaoxiangqing" label="原料详情"
                    prop="yuanliaoxiangqing">
        <span
            :style='{"fontSize":"16px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'
            v-html="ruleForm.yuanliaoxiangqing"></span>
      </el-form-item>
...
export default {
  data() {
    ...
    return {
      ...
      ro: {
        ...
        yuanliaotupian: false,
        yuanliaoleibie: false,
        jiage: false,
        shuliang: false,
        yuanliaoxiangqing: false,
      },
  ...
  methods: {
    ...
    init(id, type) {
      ...
      if (this.type == 'info' || this.type == 'else') {
        ...
        for (var o in obj) {
          ...
          if (o == 'yuanliaotupian') {
            this.ruleForm.yuanliaotupian = obj[o];
            this.ro.yuanliaotupian = true;
            continue;
          }
          if (o == 'yuanliaoleibie') {
          ...
          if (o == 'yuanliaoxiangqing') {
            this.ruleForm.yuanliaoxiangqing = obj[o];
            this.ro.yuanliaoxiangqing = true;
            continue;
          }
        }
    ...
    info(id) {
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.ruleForm.yuanliaoxiangqing = this.ruleForm.yuanliaoxiangqing.replace(reg, '../../../springboot4xt85/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      if (this.ruleForm.yuanliaotupian != null) {
        this.ruleForm.yuanliaotupian = this.ruleForm.yuanliaotupian.replace(new RegExp(this.$base.url, "g"), "");
      }
    ...
    yuanliaotupianUploadChange(fileUrls) {
      this.ruleForm.yuanliaotupian = fileUrls;
    },
  }
};
</script>
